...
// Demo items matching the design screenshot
...
export default function CartPage() {
  ...
  const [demoItems, setDemoItems] = useState(DEMO_CART_ITEMS);
  ...
  const handleValidateCoupon = async () => {
    ...
    try {
      const { data } = await orderApi.validateCoupon({
        ...
        order_amount: cart?.subtotal || 2497,
      });
      if (data.valid) {
  ...
  // Use store cart items if available; otherwise use DEMO items matching design
  const hasRealCart = cart && cart.items && cart.items.length > 0;
  const itemsList = hasRealCart ? cart.items : demoItems;

  const handleQtyChange = (product_id: number, delta: number, currentQty: number) => {
    if (hasRealCart) {
      if (delta < 0) {
      ...
      setDemoItems((prev) =>
        prev
          ...
          .filter(Boolean) as typeof DEMO_CART_ITEMS
      );
    }
  };
  ...
  const handleRemoveItem = (product_id: number) => {
    if (hasRealCart) {
      ...
      setDemoItems((prev) => prev.filter((i) => i.product_id !== product_id));
      toast.success("Item removed from cart");
    }
  };
  ...
  const rawSubtotal = itemsList.reduce((acc, i) => acc + (i.product?.price || (i as any).price || 0) * i.quantity, 0);
  const discountAmount = hasRealCart
    ? couponDiscount
    : Math.round(rawSubtotal * 0.1); // ~10% discount to match design example (₹249)
  const deliveryCharges = rawSubtotal >= 999 ? 0 : 99;
                  ...
                  <div key={item.id} className="p-3.5 flex items-center gap-3.5">
                    ...
                    <div className="flex-1 min-w-0 space-y-1">
                      ...
                      <div className="inline-flex items-center border border-[#E0DBD0] rounded-lg overflow-hidden bg-white mt-1.5">
                        <button
                          onClick={() => handleQtyChange(item.product_id, -1, item.quantity)}
                  ...
                  <div className="space-y-2">
                    ...
                    {couponDiscount > 0 ? (
                      <div className="p-3 bg-[#E8F5E9] border border-[#C8E6C9] rounded-xl flex items-center justify-between text-xs text-[#2E7D32]">
                        <div>
                          <span className="font-bold block uppercase">{couponCode || "RATNA10"}</span>
